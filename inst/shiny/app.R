...
# Registra a pasta 'inputs' para links diretos
addResourcePath("inputs", "inputs")
  ...
  "
    ...
    tabItems(
      tabItem(
        ...
        fluidRow(
          box(
            ...
            solidHeader = TRUE,
            p(
              "Se você escolher 'Usar Arquivos Predefinidos', os arquivos da pasta 'inputs/' do aplicativo serão usados automaticamente."
            ),
            checkboxInput(
              "use_predefined_files",
              "Usar Arquivos Predefinidos do Projeto Eldorado Paulista - SG.22-X-B-VI",
              value = FALSE
            )
          )
        ),
      ...
      tabItem(
        ...
        p("São apresentados os itens Mapa, Gráficos e Tabela de dados. No mapa, o usuário pode visualizar as estações de amostragem, bacias hidrográficas, unidades geológicas e rede de drenagem. Nos gráficos, é possível gerar histogramas e boxplots por litologia, boxplot geral e gráfico C-A. A tabela de dados apresenta os dados processados com as classificações."),

        h3("Arquivos de Entrada Necessários"),
...
p(tags$a(
  ...
  "https://github.com/VivianeCF/GeochemField"
        ...
        fluidRow(
          column(
            6,
            h4("Visualize as tabelas (CSV)"),
            selectInput(
              "info_select_csv",
              "Escolha um CSV:",
              choices = if (dir.exists("inputs")) list.files("inputs", pattern = "\\.csv$", full.names = FALSE) else character(0)
            ),
            actionButton("info_view_csv", "Visualizar CSV")
          ),
          column(
            6,
            h4("Visualize os atributos das feições (SHP)"),
            selectInput(
              "info_select_shp",
              "Escolha um arquivo .shp (apenas o .shp):",
              choices = if (dir.exists("inputs")) list.files("inputs", pattern = "\\.shp$", full.names = FALSE) else character(0)
            ),
            actionButton("info_view_shp", "Visualizar atributos SHP")
          )
        ),
        br(),
        downloadButton("download_inputs_zip", "Baixe todos os arquivos (ZIP)"),
        hr(),

        h3("Processamento dos Dados"),
        h4("Classificação"),
        p("O aplicativo oferece três métodos de classificação para os dados geoquímicos:"),
        tags$ul(
          tags$li(tags$strong("TIF (Tukey Inner Fence):"), " Baseado nos limiares do boxplot segundo o método Tuckey;"),
          tags$li(tags$strong("MAD (Median absolute deviation):"), " Método mais robusto baseado nos valores de mediana e mad;"),
          tags$li(tags$strong("C-A (Concentração - Área):"), " Método fractal que considera a área acumulada das bacias e teores ordenados de forma ascendente.")
        ),
        p("Para a classificação, os dados são previamente logtransformados."),

        hr(),

        h3("Fonte dos Dados de Entrada - Demo"),
        ...
        p(HTML("Disponível em: <a href='https://rigeo.sgb.gov.br/handle/doc/11525' target='_blank'>https://rigeo.sgb.gov.br/handle/doc/11525</a>.")),

        hr(),

        h3("Desenvolvimento"),
        p("Viviane Carillo Ferrari"),

        uiOutput("data_atualizacao_output"),
...
server <- function(input, output, session) {
  ...
  output$csv_inputs <- renderUI({
    if (input$use_predefined_files) {
      ...
      div(
        ...
        fileInput(
          "myjob_upload",
          "2. Escolha dados das condições analíticas",
          accept = c(
            "text/csv",
            "text/comma-separated-values,text/plain",
            ".csv"
        ...
        fileInput(
          "mylitho_upload",
          "3. Escolha dados litológicos das estações",
          accept = c(
            "text/csv",
            "text/comma-separated-values,text/plain",
            ".csv"
        ...
        fileInput(
          "mylegend_upload",
          "4. Escolha legenda para simbologia das litologias",
          accept = c(
            "text/csv",
            "text/comma-separated-values,text/plain",
            ".csv"
  ...
  output$shp_inputs <- renderUI({
    if (input$use_predefined_files) {
      ...
      div(
        fileInput(
          "mygeology_upload",
          "5. Escolha arquivos da geologia (.shp, .shx, .dbf, .prj)",
          multiple = TRUE,
          accept = c(".shp", ".dbf", ".shx", ".prj")
        ),
        fileInput(
          "mywatershed_upload",
          "6. Escolha arquivos das sub-bacias (.shp, .shx, .dbf, .prj)",
          multiple = TRUE,
          accept = c(".shp", ".dbf", ".shx", ".prj")
        ),
        fileInput(
          "myoutlet_upload",
          "7. Escolha arquivos das estações de coleta (.shp, .shx, .dbf, .prj)",
          multiple = TRUE,
          accept = c(".shp", ".dbf", ".shx", ".prj")
        ),
        fileInput(
          "mystream_upload",
          "8. Escolha arquivos da rede de drenagem (.shp, .shx, .dbf, .prj)",
          multiple = TRUE,
          accept = c(".shp", ".dbf", ".shx", ".prj")
        )
      )
  ...
  mylegend_upload <- reactive({
    if (input$use_predefined_files) {
      ...
      df <- read_csv2(
        input$mylegend_upload$datapath,
        locale = locale(encoding = "latin1")
      )
    }
  ...
  lito_geo_processed <- reactive({
    ...
    message(sprintf(
      "lito_geo_processed: RGB valid for %d of %d features",
      sum(!is.na(lito_geo$RGB)),
      nrow(lito_geo)
    ))
    lito_geo
  })
  ...
  observeEvent(
    c(mydata_processed(), myjob_upload()),
    {
      # Ou observeEvent(c(mydata, myjob), {
      myjob <- myjob_upload()
      mydata <- mydata_processed()
      variances <- sapply(mydata, function(col) {
        if (is.numeric(col)) var(col, na.rm = TRUE) else NA
      })
      # unique_counts <- sapply(mydata, function(col) if (is.numeric(col)) length(unique(col[!is.na(col)])) else NA) # Não usado
      m <- sapply(mydata, function(col) {
        if (is.numeric(col)) median(col, na.rm = TRUE) else NA
      })
      mad <- sapply(mydata, function(col) {
        if (is.numeric(col)) mad(col, na.rm = TRUE) else NA
      })
  ...
  cores_fixas_geo <- reactive({
    ...
    codes <- if ("Geo_cod" %in% names(lg)) {
      as.character(lg$Geo_cod)
    } else if ("SIGLA" %in% names(lg)) {
      as.character(lg$SIGLA)
    } else {
      character(0)
    }
    cols <- if ("RGB" %in% names(lg)) {
      as.character(lg$RGB)
    } else {
      rep(NA_character_, length(codes))
    }
    if (length(codes) == 0) {
      named <- setNames(character(0), character(0))
    } else {
      named <- setNames(cols, codes)
    }
    named
  })
  ...
  output$mymap <- renderLeaflet({
    ...
    # Criar o mapa
    leaflet(
      bacias_classificadas,
      options = leafletOptions(minZoom = 5, maxZoom = 22)
    ) |>
  ...
  observeEvent(
    ...
    ),
    {
        ...
        proxy |>
          addLegend(
            ...
            title = paste(
              input$variable,
              " (",
              myjob[myjob$EL == input$variable, "UN"],
              ")"
            ),
      ...
      if ("estações" %in% input$mymap_groups) {
        ...
        proxy |>
          addLegend(
            ...
            pal = colorFactor(
              palette = "transparent",
              domain = dados_classificados$Classe
            ), # Paleta fictícia
            #        pal = pal_legenda,
      ...
      if (
        "estações" %in% input$mymap_groups & "bacias" %in% input$mymap_groups
      ) {
        # Criar os rótulos da legenda com ícones personalizados
        labels <- c()
        for (i in seq_along(limiares)) {
      ...
      if ("geologia" %in% input$mymap_groups) {
        ...
        itens_legenda_html <- paste0(
          ...
          cores_legenda,
          "; display: inline-block; width: 10px; height: 10px; margin-right: 5px;'></span>",
          nomes_legenda,
          "</div>",
          collapse = ""
        )
  ...
  observeEvent(c(input$variable, input$classification_type), {
    ...
    leafletProxy("mymap") |>
      setView(
        lng = map_state$center$lng,
        lat = map_state$center$lat,
        zoom = map_state$zoom
      )
  })
  ...
  output$dataTable <- DT::renderDataTable({
    ...
    dados_classificados <- left_join(
      mydata,
      processed_data()$classificados,
      by = "VALUE"
    )

    dados_classificados <- left_join(
      sf::st_drop_geometry(pt)[, -ncol(pt)],
      dados_classificados,
      by = "VALUE"
    )
  ...
  observeEvent(input$info_view_csv, {
    req(input$info_select_csv)
    csv_path <- file.path("inputs", input$info_select_csv)
    df <- tryCatch(
      readr::read_csv2(csv_path, locale = readr::locale(encoding = "latin1")),
      error = function(e) {
    ...
    req(df)
    showModal(modalDialog(
      title = paste("Visualizando:", input$info_select_csv),
      DT::dataTableOutput("modal_csv_table"),
      size = "l",
      easyClose = TRUE,
      footer = modalButton("Voltar")
    ))
    output$modal_csv_table <- DT::renderDataTable({
      DT::datatable(
        df,
  ...
  # Modal para visualizar atributos do SHP (sem geometria)
  observeEvent(input$info_view_shp, {
    req(input$info_select_shp)
    shp_path <- file.path("inputs", input$info_select_shp)
    shp <- tryCatch(sf::st_read(shp_path, quiet = TRUE), error = function(e) {
      showModal(modalDialog(
        title = "Erro ao ler SHP",
        paste0(e$message),
        easyClose = TRUE
      ))
      return(NULL)
    ...
    req(shp)
    attrs <- sf::st_drop_geometry(shp)
    showModal(modalDialog(
      title = paste("Atributos de:", input$info_select_shp),
      DT::dataTableOutput("modal_shp_table"),
      size = "l",
      easyClose = TRUE,
      footer = modalButton("Voltar")
    ))
    output$modal_shp_table <- DT::renderDataTable({
      DT::datatable(
        attrs,
        options = list(pageLength = 15, scrollX = TRUE),
        rownames = FALSE
  ...
  # Download zip com todo o conteúdo da pasta inputs
  output$download_inputs_zip <- downloadHandler(
    filename = function() paste0("inputs_", Sys.Date(), ".zip"),
    content = function(file) {
      ...
      if (length(files) == 0) {
        stop("Nenhum arquivo encontrado na pasta 'inputs/'.")
      }
      # tenta criar zip com utils::zip
